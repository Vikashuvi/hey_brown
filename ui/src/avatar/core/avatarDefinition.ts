...
export const validateAvatarDefinition = (value: unknown): ValidationResult<AvatarDefinition> => {
  if (!value || typeof value !== 'object') {
    return { ok: false, errors: [{ path: '', code: 'invalid_type', message: 'Expected an object' }] }
  }
  const structuralErrors = inspectMaterializedValue(value)
  if (structuralErrors.length) return { ok: false, errors: structuralErrors }
  const errors = semanticErrors(value as AvatarDefinition)
  return errors.length ? { ok: false, errors } : { ok: true, value: cloneAndFreeze(value as AvatarDefinition) }
}
