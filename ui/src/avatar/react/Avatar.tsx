...
export type AvatarProps = {
  ...
  style?: CSSProperties
  ariaLabel?: string
  /** Enables 3D liquid crystal glossy highlights and depth shading. */
  glossy?: boolean
    ...
    >
      <svg className="bs-avatar__svg" viewBox="-150 -150 300 300" aria-hidden="true">
        <defs>
          ...
          {/* Volumetric spherical depth shading */}
          <radialGradient
            id={`${clipId}-depthShade`}
            cx="35%"
            cy="35%"
            r="65%"
            fx="30%"
          ...
          >
            ...
            <stop offset="78%" stopColor="#050212" stopOpacity="0.22" />
            <stop offset="100%" stopColor="#050212" stopOpacity="0.55" />
          </radialGradient>
          {/* Top-left intense specular gleam */}
          <radialGradient
            id={`${clipId}-glossHighlight`}
            cx="-35"
            cy="-45"
            r="80"
            fx="-45"
          ...
          {/* Glass crescent curvature highlight */}
          <linearGradient id={`${clipId}-glossCrest`} x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="#ffffff" stopOpacity="0.55" />
            <stop offset="35%" stopColor="#ffffff" stopOpacity="0.16" />
            <stop offset="100%" stopColor="#ffffff" stopOpacity="0" />
          </linearGradient>
          {/* Soft ambient bottom rim light bounce */}
          <radialGradient
            id={`${clipId}-rimLight`}
            cx="0"
            cy="110"
        ...
        {/* 3D Volumetric depth & shading layer */}
        {glossy && (
          <g clipPath={`url(#${clipId})`} pointerEvents="none">
            <circle cx="0" cy="0" r="130" fill={`url(#${clipId}-depthShade)`} />
          </g>
        )}
        ...
        {/* Top Glossy Glass Finish & Specular Highlights */}
        {glossy && (
          <g clipPath={`url(#${clipId})`} pointerEvents="none">
            {/* Top-left specular glint */}
            <circle cx="-35" cy="-45" r="80" fill={`url(#${clipId}-glossHighlight)`} />
            {/* Glass crescent curvature reflection */}
            <ellipse
              cx="-20"
              cy="-60"
              rx="65"
              ...
              fill={`url(#${clipId}-glossCrest)`}
            />
            {/* Ambient rim light */}
            <circle cx="0" cy="0" r="130" fill={`url(#${clipId}-rimLight)`} />
          </g>
        )}
        {Array.from({ length: bodyPathSlots }, (_, index) => (
          <path
